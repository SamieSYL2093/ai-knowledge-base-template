...
A：先记住一条：**别双击**。Windows 上双击 `.md` 默认用记事本打开，你会看到满屏 `#`、`|`、`**`，像文件坏了一样——**它没坏，只是没渲染**。`.md` 是 Markdown，一种纯文本格式，渲染之后才是你熟悉的样子。
...
| **装个阅读器** | 装个 MD 阅读器（如 mdview，开源免费；Typora / Obsidian / VS Code 也行），装完双击就是排版好的样子 | 需要经常翻本地 MD 的人 |
