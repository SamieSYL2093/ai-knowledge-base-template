...
**1. 重复的工作，做成 skill**
同一件事做了第二次，就该固化成 skill 或规范——**做两次，就升级**，从此不再重复造轮子。

**2. 每次出错，想防呆，升级 skill**
错误不是用来道歉的，是用来升级系统的：

`错误发生 → 记录 → 根因分析 → 防呆措施 → 升级进 skill/规范 → 下次不再犯`
...
**错误记录在哪？** 记在**子项目**里，一句话四要素（见档案原则 11）；不建中心台账（没人维护会变僵尸）。哪个教训跨项目通用，才升级进知识库——从行李升地图。

**4. 防记忆膨胀：内容放对层**
知识库总纲只存理念/原则/台账，具体内容一律下沉到子项目。库越长，AI 每次读的就越多、越贵、越慢——定期审视：这段内容是"地图"还是"行李"？是行李就下沉（档案原则 12）。
