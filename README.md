...
> 💡 **先搞清楚一件事**：这套 MD 文件是**给 AI 看的**，不是给你看的。你不需要读懂任何一个文件，甚至连"填写档案"都是 AI 帮你干的——你要做的只有下面两步。

## 🚶 两步上手（就两步，不用懂 Git、不用懂技术、不用改任何文件）

### 第一步：文件夹放哪？—— 和你的项目文件夹**平行**放
...
├── 项目D/              # 生产或财务数据分析
├── 项目E/              # 资料整理
└── 知识库/  ← 就放在这，和项目们平行，一个库统一管所有项目
```

**"项目"泛指一切需要用 AI 辅助干活的事**：软件开发、教材课件开发、个人学习规划、生产或财务数据分析、资料整理……**很多人不知道自己用得上 AI——记住：你日常任何需要查资料、写东西、算数据、整理信息的事，都可以让 AI 帮你干，那就算一个"项目"**。

> 怎么拿到这个文件夹：点本页右上角绿色「Use this template」创建你自己的仓库，或直接下载 ZIP 解压。

### 第二步：把知识库里的 `_入项指引.md` 发给电脑端 AI

**电脑端 AI = 能读写本地文件的 AI**（豆包桌面端、Kimi、Claude Code、OpenClaw 等）。两种发法，效果一样：

1. 把 `_入项指引.md` 的内容复制发给它；或者
2. 直接把知识库文件夹的路径复制给它（例如 `D:\你的工作目录\知识库`）

**然后 AI 就秒懂这套知识库了**——它会自己读完所有规则，按你的习惯干活。你的"用户档案"也不用自己填，跟 AI 说一句"读 1-01_用户档案.md，边聊边帮我填"就行。

> 🙋 **你不懂的，直接问 AI**——包括"这个 skill 怎么用""下一步该干嘛"。说明书是写给 AI 的，AI 再翻译给你听。
...
├── _入项指引.md               # 🎫 【唯一要发给AI的文件】AI读完秒懂整套体系
...
**Q：只用网页版 AI（豆包网页版/Kimi网页版/ChatGPT）能用吗？**
A：体验会打折。这套体系的核心设计假设 AI 能读写本地文件——桌面端 AI（豆包桌面端/Kimi/Claude Code/OpenClaw）是满血体验：AI 自动读文件、改文件、Git 提交一气呵成。网页版 AI 读不了你的文件夹，只能把 `_入项指引.md` 的内容粘贴给它让它懂规矩，干活成果再靠你手动搬运。建议至少装一个桌面端 AI，其他辅助用网页版。
