...
> 🪃 **你甚至不用先有业务**——用着用着，它就会帮你长出项目来（详见下文「它会自己长项目」）。
...
**定位**：一套**写给 AI 看的 skill 规范**。人不用学、不用配——**你只跟 AI 对话，AI 替你把规矩、记忆、流程全扛起来**。

**理念（4 句话）**：
1. 🤝 **人只对话、AI 全扛**——降低你的认知负荷是 AI 的 KPI，不是你的义务
2. 🔄 **双向奔赴**——你借对话长成"知识树"，AI 被充分授权释放能力，互为因果
3. 🗺️ **靠结构不靠人记**——单一事实来源 + 防呆闭环，把经验焊进文件，不靠谁记性
4. 📦 **简单化封装**——体系再大，落到你面前就是一句"他开口、你办事"

## 📂 你只需认这 4 个文件（其余进阶按需读）

| 文件 | 干什么 | 你要碰吗 |
|------|--------|---------|
| `_入项指引.md` | 【唯一要发给 AI 的】AI 读完秒懂整套体系，先懂你再办事 | 只复制它发给 AI |
| `1-01_用户档案.md` | 你的身份/偏好/规则——**AI 代你填**，你只核对 | 不碰，AI 填 |
| `1-09_防呆与持续改善.md` | 体系自愈：踩坑→5 步闭环变机制 | 不碰，AI 用 |
| 单轨交底 | 写在 `_入项指引` 里：新人走首访链、熟人一行带过 | 不碰 |

> 剩下的 `1-03`~`1-08`、指挥中心模板都是**进阶可选**——多 AI 派活、自我进化、机制清单……上手不用读，想深究再翻。

## 🪃 它会自己长项目（回旋镖机制）

**你不用先有业务。** 这套模板的业务层（`10-19_业务流程` / `20-29_工具技能`）是**刻意留空的生长位**——不预填假想业务，但随时能长。

用着用着，你会冒出"原来我还能用 AI 干这个"：辅导计划、教材课件、改善提案、甚至这套知识库系统本身。这些新项目是你自己的下游实例，在真实场景里跑、踩坑，再通过闭环**回流**改进模板：

```
空白种子 → 你对话AI / 被AI教着用AI → 长出新项目 X
   → X 踩坑 (业务坑 / 系统坑 / 模板洞)
   → 业务坑写进 X 自己的文件 / 系统坑进防呆台账 / 模板洞提 Issue 改模板
   → 模板更聪明 → 你更敢用 AI 干新事 → 更多 X → 回旋镖转起来
```

扔出去的是空白种子，飞回来的是更聪明的模板。**它不只是一套知识库，是 AI 时代你的能力放大器。**
...
> 🎯 **最小可用集**：单 AI 轻量用，只需要 `_入项指引.md`、`1-01`、`1-02`、`3-01`、`项目模板/` 五样——其余全是按需启用的可选件，不用提前看。
...
├── check.py                   # ✅ 发布/同步前检查脚本：脱敏+版本一致+旧术语残留+文件头，全绿才发
...
│
├── 10-19_业务流程/           # 🌱 【生长位·刻意留空】你的行业知识在这长：辅导计划/教材/PPT规范/业务SOP，按需长不预填
├── 20-29_工具技能/           # 🌱 【生长位·刻意留空】工具手册在这长：HTML课件规范/SVG写法/踩坑记录，按需长不预填
│
...
模板持续进化（当前 v1.0.9）。记住一句话：**机制随版本走，台账各自长，行业包各自填。**

- **你的东西永远是你的**：clone 之后，`1-01` 档案、防呆台账、`10-19/20-29` 行业内容、各项目——全是你的本地文件，放心长，模板升级永远撞不上
- **每次发版都有变更清单**：[CHANGELOG.md](./CHANGELOG.md) 里写着每个版本改了哪些文件。升级 = 按清单把对应机制文件抄进你的知识库，你的本地内容不受影响。不想跟的版本直接跳过
- **让 AI 帮你升级（推荐）**：把 [CHANGELOG.md](./CHANGELOG.md) 或 Release 链接发给你的桌面端 AI，说一句"按 CHANGELOG 把机制改进抄到我的知识库，我的本地内容别动"，AI 会自己对比、替换、提交
