...
| `指挥中心.md` + `gen_html.py` | 人看的唯一入口：**让 AI 跑** `python gen_html.py` 生成 `指挥中心.html`，浏览器设书签 | 只看 HTML |
...
| 方式 | 怎么做 | 适合谁 |
|------|--------|--------|
| **不看**（推荐） | 让 AI 跑 `python gen_html.py` 生成 `指挥中心.html`，用浏览器看 | 绝大多数人——本模板本来就是这么设计的 |
| **网页上看** | 在 GitHub 仓库页面点开 `.md`，网页会自动渲染成排版好的文档 | 看已经推到远程的文件 |
| **装个阅读器** | 装个 MD 阅读器（mdview，开源免费；Typora / Obsidian 也行），装完双击 `.md` 就是排版好的样子 | 需要经常翻本地 MD 的人 |

> 不想装东西、也不想跑脚本？把文件拖进在线的 MD 查看器也能渲染——但那是把文件交给第三方网站，涉密内容别用。

**Q：我需要打开那些 MD 文件改东西吗？**
A：不需要。MD 文件是给 AI 看的，连档案都是聊着天 AI 就帮你填好了。你唯一常看的只有 `指挥中心.html`（想自己翻一眼 MD，见上一条）。

**Q：我只有网页版 AI 能用吗？**
A：**不行。** 这套体系假设 AI 能读写你本地文件——读档案、改清单、生成仪表盘、Git 提交都依赖这个能力。请用上文"两步上手"里列的那类桌面端 AI。
