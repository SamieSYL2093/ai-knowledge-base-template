...
<p align="center">
  ...
  <br/>
  <img src="https://img.shields.io/badge/%E5%85%AD%E7%BB%B4%E8%87%AA%E8%AF%84-76/100%20%28%E7%9F%AD%E6%9D%BF%3A%E5%8D%95%E4%B8%80%E6%BA%90%29-orange?style=flat-square" alt="六维自评">
</p>

---

> 📊 **本模板六维自评：76 分（短板：单一源）**。复跑：`python scripts/kb_audit.py .`——六维体检（单一源/放对层/可找到/不过期/不冗余/可执行），总分取短板，只报不修。
> （08-31 注：README 自评行本身会命中"单一源"检查的关键词——分数含自证代价，如实记录不凑分。）
...
>
> 📖 **建议先装个 MD 阅读器**（mdview，开源免费，装一次就行）——以后双击任何 `.md` 就是排版好的文档。
> **没装之前别双击**：Windows 会用记事本打开，满屏 `#`、`|`、`**`，看着像文件坏了（详见下方常见问题）。
...
> 之后遵守档案里的基本原则干活。MD 文件是给你（AI）看的；**人用 mdview 双击看 MD**——不再生成 HTML 投影，MD 就是最终形态，永远最新，不用记着重新生成。
...
模板不只管"组织"，还管"进化"。两条做法（原则全文见档案原则 6 / 10 / 11）：

- **做两次就升级** —— 同一件事做了第二次，就固化成 skill 或规范，不再重复造轮子
- **每次出错都升级系统** —— 错误不是用来道歉的：`错误发生 → 记录 → 根因分析 → 防呆措施 → 升级进 skill/规范 → 下次不再犯`

**防呆三级，能上一级不上下一级**

| 级别 | 做法 | 性质 |
|------|------|------|
| 一级 · 根源防呆 | 从系统/设计层面让错误不可能发生（系统性思考，问根本） | 最优，力气花在这 |
| 二级 · 流程防呆 | 在操作过程/规范上设防（检查清单、步骤约束） | 次优 |
| 三级 · 事后检查 | 错已发生，靠检查发现补救 | 本质是浪费的动作——提醒你别停在这 |

> 🔗 **配套工具（同作者）**：知识库越用越胖时，用 `kb-slim-audit` 做瘦身审查（SkillHub 搜索安装）——AI 输出「删/并/压/移/指针化」可执行清单，人只拍板。
...
| `指挥中心.md` | 人看的唯一入口：装个 mdview，双击这个文件就是排版好的仪表盘 | 双击看 |

> 知识库只存"地图"（规范 + 登记），项目内容（"行李"）在各项目目录自治。
> 想加自己的业务规范（SOP、模板、行业知识）？直接新建 MD 文件（如 `2-01_你的规范.md`），文件头带三行说明（用途/关键词/更新）即可。
...
**Q：知识库里那些 `.md` 文件，我怎么打开看？**
A：先记住一条：**别双击**。Windows 自带记事本，双击 `.md` 多半就用它打开了（或弹窗问你要用什么程序）——你会看到满屏 `#`、`|`、`**`，像文件坏了一样。**它没坏，只是没渲染**。`.md` 是 Markdown，一种纯文本格式，渲染之后才是你熟悉的样子。

**装一个 MD 阅读器**，这是本模板推荐的做法（装一次，以后双击任何 `.md` 都是排版视图）：

| 做法 | 怎么做 | 适合谁 |
|------|--------|--------|
| **装阅读器**（推荐） | 装 mdview（开源免费），装完双击 `.md` 直接看；Typora / Obsidian 也行 | 所有人——本模板就是按「人看 MD」设计的 |
| **网页上看** | 在 GitHub 仓库页面点开 `.md`，网页会自动渲染成排版好的文档 | 看已经推到远程的文件 |
| **装都懒得装** | 直接问你的 AI："我的指挥中心现在什么状态？" | 只想拿结论、不想翻文件的人 |

> 不想装东西又想看本地文件？把文件拖进在线的 MD 查看器也能渲染——但那是把文件交给第三方网站，涉密内容别用。

**Q：我需要打开那些 MD 文件改东西吗？**
A：不需要。档案、清单这些都由 AI 维护，连内容都是聊着天 AI 就帮你填好了。你常看的只有 `指挥中心.md`——装个 mdview 双击打开就行（见上一条）。

**Q：我只有网页版 AI 能用吗？**
A：**不行。** 这套体系假设 AI 能读写你本地文件——读档案、改清单、维护仪表盘、Git 提交都依赖这个能力。请用上文"两步上手"里列的那类桌面端 AI。
